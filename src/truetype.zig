...
pub const GlyphBitmapExt = struct {
    pixels: []const u8,
    width: i32,
    height: i32,
    off_x: i32,
...
pub const GlyphCache = struct {
    ...
    pub fn get(gc: *GlyphCache, alloc: Allocator, char: u8) !GlyphBitmapExt {
        ...
        if (!gly.found_existing) {
            ...
            const bounds = try gc.ttf.glyphBitmap(alloc, &bm, codepoint, gc.size, gc.size);
            gly.value_ptr.* = .{
        ...
        return gly.value_ptr.*;
    }

    pub fn raze(gc: *GlyphCache, alloc: Allocator) void {
        var vitr = gc.map.valueIterator();
        while (vitr.next()) |each| {
            alloc.free(each.pixels);
        }
        gc.map.deinit(alloc);
    }
};
...
fn rasterizeSortedEdges(
    ...
    var scanline_buffer: [1350]f32 = undefined;
...
pub fn debugGlyph(rendered: GlyphBitmapExt) void {
    std.debug.print("                 |", .{});
    for (0..rendered.width) |i| std.debug.print("{d:4}", .{i});
    std.debug.print("\n", .{});
    std.debug.print("                 |", .{});
    for (0..rendered.width) |_| {
...
test "smoke test render all" {
    const debug_print_timing = false;
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();

    const font_file_name: ?[]const u8 = null;
    if (font_file_name == null) return error.SkipZigTest;
    const embed = @embedFile("font.ttf");
    const font: []align(2) u8 = try alloc.alignedAlloc(u8, 2, embed.len);
    @memcpy(font, embed);
    defer alloc.free(font);
    const ttf = try load(font);

    for (0x21..0x7f) |char| {
        const glyph = ttf.codepointGlyphIndex(@intCast(char)) orelse {
